...
def create_movie_poster(title):
    ...
    for word in words:
        ...
        max_chars = 12 if len(title) > 25 else 16
    ...
    if len(title_lines) > 3 or any(len(line) > 18 for line in title_lines):
        font_size = 50
        try:
            font = ImageFont.truetype("/System/Library/Fonts/Helvetica.ttc", font_size)
        except:
    ...
    # Ensure text stays within poster bounds (leave 80px margin top and bottom)
    max_y = 750 - 80
    min_y = 80
    ...
    # Ensure start_y is within bounds
